...
dados = data.frame(group = c("A ", "B ", "C ", "D "), value = c(32,62,56,67))
...
fatias <- c(4, 12, 14, 16, 8)
paises <- c("Brasil", "Estados Unidos", "Alemanha", "Reino Unido", "Espanha")
pie3D(fatias, labels = paises, main = "Leitura de Livros por Pessoa/Ano")
...
valor = c(13,5,22,121,11,7,3,1,23)
data = data.frame(grupo, subgrupo, valor)

# Labels
treemap(data, index=c("grupo", "subgrupo"),
        vSize = "valor", 
        type = "index",
